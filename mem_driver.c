...
static uint32_t mem_major = MEMDEV_MAJOR;   // 主设备号
static uint32_t mem_minor = 0;				// 次设备号

static struct mem_dev *mem_devp;
static struct cdev cdev;
static dev_t devno;   //设备编号， 内部表达
...
static ssize_t mem_read(struct file *filp, char __user *buf, size_t size, loff_t *ppos)
{
	...
	if (count > dev->size - p)    //整数溢出， 越界读。 注意 size 是 0x5axxxxxx
		count = dev->size - p;
...
static ssize_t mem_write(struct file *filp, const char __user *buf, size_t size, loff_t *ppos)
{
	...
	if (count > dev->size - p)   //注意 size 是 0x5axxxxxx
		count = dev->size - p;
...
static long mem_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if(data.len <= 0 || data.len >= 0x1000000)   //0~16M
		return -EINVAL;
...
static int __init memdev_init(void)
{
	...
	if (mem_major) {
		...
		ret = register_chrdev_region(devno, MEMDEV_NR_DEVS, "memdev");  //获取 3个设备编号 ，从 devno 开始，  param3：设备名称
		if (ret < 0) {
			pr_err("register_chrdev_region failed %d\n", ret);
			goto out;
		}
	} else {
		ret = alloc_chrdev_region(&devno, mem_minor, MEMDEV_NR_DEVS, "memdev");  //动态分配恰当的主设备号（推荐）
		if (ret < 0) {
			pr_err("alloc_chrdev_region failed %d\n", ret);
			goto out;
		}
		mem_major = MAJOR(devno);   //获取主设备号，应为这里采用动态申请设备号
	}

	cdev_init(&cdev, &mem_fops);      //给设备 cdev 绑定操作
	cdev.owner = THIS_MODULE;

	ret = cdev_add(&cdev, devno, MEMDEV_NR_DEVS);  // 将设备对象 cdev 添加到系统中（linux 内核）, 建立起 System Call呼叫驱动程序
	if (ret < 0) {
		pr_err("alloc_chrdev_region failed %d\n", ret);
		goto fail_cdev_add;
	}
